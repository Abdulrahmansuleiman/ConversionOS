---
source_file: "launchops-portal/tsconfig.app.json"
type: "code"
community: "compilerOptions"
location: "L2"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/compilerOptions
---

# compilerOptions

## Connections
- [[allowImportingTsExtensions_2]] - `contains` [EXTRACTED]
- [[jsx_1]] - `contains` [EXTRACTED]
- [[launchops-portaltsconfig.app.json]] - `contains` [EXTRACTED]
- [[lib_2]] - `contains` [EXTRACTED]
- [[module_2]] - `contains` [EXTRACTED]
- [[moduleDetection_2]] - `contains` [EXTRACTED]
- [[moduleResolution_2]] - `contains` [EXTRACTED]
- [[noEmit_2]] - `contains` [EXTRACTED]
- [[noFallthroughCasesInSwitch_2]] - `contains` [EXTRACTED]
- [[noUncheckedSideEffectImports_2]] - `contains` [EXTRACTED]
- [[noUnusedLocals_2]] - `contains` [EXTRACTED]
- [[noUnusedParameters_2]] - `contains` [EXTRACTED]
- [[skipLibCheck_2]] - `contains` [EXTRACTED]
- [[strict_2]] - `contains` [EXTRACTED]
- [[target_2]] - `contains` [EXTRACTED]
- [[tsBuildInfoFile_2]] - `contains` [EXTRACTED]
- [[useDefineForClassFields_1]] - `contains` [EXTRACTED]
- [[verbatimModuleSyntax_2]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/compilerOptions