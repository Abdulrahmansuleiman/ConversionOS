---
source_file: "bloomline-dashboard/src/components/ui.tsx"
type: "code"
community: "Dashboard UI Components"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Dashboard_UI_Components
---

# bloomline-dashboard/src/components/ui.tsx

## Connections
- [[Card_3]] - `contains` [EXTRACTED]
- [[CardHeader_1]] - `contains` [EXTRACTED]
- [[CardSub_1]] - `contains` [EXTRACTED]
- [[CardTitle_1]] - `contains` [EXTRACTED]
- [[ChannelPill()_1]] - `contains` [EXTRACTED]
- [[EmptyCell_1]] - `contains` [EXTRACTED]
- [[Pill]] - `contains` [EXTRACTED]
- [[StatusPill()_1]] - `contains` [EXTRACTED]
- [[TONE_COLORS_1]] - `contains` [EXTRACTED]
- [[Table_2]] - `contains` [EXTRACTED]
- [[Td_1]] - `contains` [EXTRACTED]
- [[Th_1]] - `contains` [EXTRACTED]
- [[Theme_1]] - `imports` [EXTRACTED]
- [[Tone_1]] - `contains` [EXTRACTED]
- [[Tr_1]] - `contains` [EXTRACTED]
- [[bloomline-dashboardsrctabsBookingsTab.tsx]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrctabsCalendarTab.tsx]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrctabsConversationsTab.tsx]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrctabsDashboardTab.tsx]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrctabsFollowUpsTab.tsx]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrctabsHumanTransfersTab.tsx]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrctabsSettingsTab.tsx]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrctheme.ts]] - `imports_from` [EXTRACTED]
- [[channelColor()_1]] - `contains` [EXTRACTED]
- [[statusTone()_1]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Dashboard_UI_Components