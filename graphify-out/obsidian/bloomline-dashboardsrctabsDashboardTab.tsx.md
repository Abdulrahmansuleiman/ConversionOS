---
source_file: "bloomline-dashboard/src/tabs/DashboardTab.tsx"
type: "code"
community: "Chart and Toggle Components"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Chart_and_Toggle_Components
---

# bloomline-dashboard/src/tabs/DashboardTab.tsx

## Connections
- [[CardHeader_1]] - `imports` [EXTRACTED]
- [[CardSub_1]] - `imports` [EXTRACTED]
- [[CardTitle_1]] - `imports` [EXTRACTED]
- [[ConversionDonut()_1]] - `imports` [EXTRACTED]
- [[DarkCard_1]] - `contains` [EXTRACTED]
- [[DarkCardHeader_1]] - `contains` [EXTRACTED]
- [[DarkSub_1]] - `contains` [EXTRACTED]
- [[DarkTitle_1]] - `contains` [EXTRACTED]
- [[DashboardTab()_1]] - `contains` [EXTRACTED]
- [[DonutSegment_1]] - `imports` [EXTRACTED]
- [[EmptyState()_1]] - `imports` [EXTRACTED]
- [[ErrorState()]] - `imports` [EXTRACTED]
- [[KPI_META_1]] - `contains` [EXTRACTED]
- [[KpiCard()_2]] - `imports` [EXTRACTED]
- [[KpiGrid_2]] - `contains` [EXTRACTED]
- [[KpiKey_1]] - `imports` [EXTRACTED]
- [[KpiRow_1]] - `contains` [EXTRACTED]
- [[LeadBar_1]] - `contains` [EXTRACTED]
- [[LeadBarTrack_1]] - `contains` [EXTRACTED]
- [[LeadCard_1]] - `contains` [EXTRACTED]
- [[LeadList_1]] - `contains` [EXTRACTED]
- [[LeadName_1]] - `contains` [EXTRACTED]
- [[LeadRow_1]] - `contains` [EXTRACTED]
- [[LeadScore_1]] - `contains` [EXTRACTED]
- [[Rank_1]] - `contains` [EXTRACTED]
- [[SampleDataBadge()_1]] - `imports` [EXTRACTED]
- [[SectionTitle_2]] - `contains` [EXTRACTED]
- [[TIMEFRAME_LABELS_3]] - `contains` [EXTRACTED]
- [[Theme_1]] - `imports` [EXTRACTED]
- [[Timeframe_1]] - `imports` [EXTRACTED]
- [[TimeframeToggle()]] - `imports` [EXTRACTED]
- [[TopBar_1]] - `contains` [EXTRACTED]
- [[TopRight_1]] - `contains` [EXTRACTED]
- [[TopRow_2]] - `contains` [EXTRACTED]
- [[WeeklyTrend()_1]] - `imports` [EXTRACTED]
- [[bloomline-dashboardsrcApp.tsx]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrccomponentsEmptyState.tsx]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrccomponentsErrorState.tsx]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrccomponentsKpiCard.tsx]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrccomponentsSampleDataBadge.tsx]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrccomponentsTimeframeToggle.tsx]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrccomponentschartsConversionDonut.tsx]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrccomponentschartsWeeklyTrend.tsx]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrccomponentsui.tsx]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrchooksuseEvents.ts]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrchooksuseKpis.ts]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrchooksuseTopLeads.ts]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrchooksuseTrend.ts]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrctheme.ts]] - `imports_from` [EXTRACTED]
- [[bloomline-dashboardsrctypesevents.ts]] - `imports_from` [EXTRACTED]
- [[useEvents()]] - `imports` [EXTRACTED]
- [[useKpis()_1]] - `imports` [EXTRACTED]
- [[useTopLeads()_1]] - `imports` [EXTRACTED]
- [[useTrend()_1]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Chart_and_Toggle_Components