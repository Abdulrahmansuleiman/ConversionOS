---
source_file: "bloomline-dashboard/src/components/Sidebar.tsx"
type: "code"
community: "Dashboard Sidebar"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Dashboard_Sidebar
---

# bloomline-dashboard/src/components/Sidebar.tsx

## Connections
- [[Aside_1]] - `contains` [EXTRACTED]
- [[Avatar]] - `contains` [EXTRACTED]
- [[BloomMark()]] - `contains` [EXTRACTED]
- [[Brand_1]] - `contains` [EXTRACTED]
- [[BrandName]] - `contains` [EXTRACTED]
- [[BuiltBy]] - `contains` [EXTRACTED]
- [[Company]] - `contains` [EXTRACTED]
- [[NAV_ITEMS]] - `contains` [EXTRACTED]
- [[Nav_2]] - `contains` [EXTRACTED]
- [[NavButton]] - `contains` [EXTRACTED]
- [[Profile]] - `contains` [EXTRACTED]
- [[ProfileMeta]] - `contains` [EXTRACTED]
- [[ProfileName]] - `contains` [EXTRACTED]
- [[ProfileRole]] - `contains` [EXTRACTED]
- [[Sidebar()_1]] - `contains` [EXTRACTED]
- [[SidebarProps]] - `contains` [EXTRACTED]
- [[TabId]] - `contains` [EXTRACTED]
- [[bloomline-dashboardsrcApp.tsx]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Dashboard_Sidebar