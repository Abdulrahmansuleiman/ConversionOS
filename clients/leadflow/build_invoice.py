# -*- coding: utf-8 -*-
"""
build_invoice.py — LeadFlow.io invoice (INV-2026-002)

Rebuilds templates/Invoice Template.pdf structure faithfully, filled with
facts from clients/leadflow/facts.json (verbatim).
...
# Paths
# --------------------------------------------------------------------------
BASE_DIR = os.path.dirname(os.path.abspath(__file__))          # clients/leadflow
...
TERMS_HEADING_TOP = 554
TERMS_BODY_TOP = 574
TERMS_MAX_W = 272.0                         # body width before signature block
...
def main():
    ...
    c.setTitle("INVOICE — LeadFlow.io — INV-2026-002")
    ...
    text(LEFT_X, 166, "Daniel Whitfield", font=FONT_BOLD, size=16, color=BLACK)
    # decorative underline under client name (as in template)
    name_w = pdfmetrics.stringWidth("Daniel Whitfield", FONT_BOLD, 16)
    c.setStrokeColor(BLUE)
    c.setLineWidth(2)
    c.line(LEFT_X, PAGE_H - 190.5, LEFT_X + name_w * 0.6, PAGE_H - 190.5)
    text(LEFT_X, 199, "LeadFlow.io", font=FONT_MEDIUM, size=10, color=BLACK)
    text(LEFT_X, 212, "+44 7911 123456", font=FONT_MEDIUM, size=10, color=BLACK)
    text(LEFT_X, 225, "24 Baker Street, Marylebone, London,", font=FONT_REGULAR, size=9, color=BLACK)
    text(LEFT_X, 237, "W1U 6TU, UK", font=FONT_REGULAR, size=9, color=BLACK)
    text(LEFT_X, 263, "Marketing agency — paid ads for local businesses and ecommerce brands",
         font=FONT_REGULAR, size=9, color=BLACK)

    # Middle column — Date / document numbers
    text(DATE_X, 148, "Date : August 22, 2026", font=FONT_BOLD, size=10, color=BLACK)
    text(MID_X, 176, "Invoice No : INV-2026-002", font=FONT_BOLD, size=10, color=BLACK)
    text(MID_X, 195, "Proposal No : PRO-2026-002", font=FONT_BOLD, size=10, color=BLACK)
    text(MID_X, 210, "Proposal Date : August 22, 2026", font=FONT_BOLD, size=10, color=BLACK)
    text(MID_X, 225, "Contract No : CON-2026-002", font=FONT_BOLD, size=10, color=BLACK)
    text(MID_X, 240, "Receipt No : RCP-2026-002", font=FONT_BOLD, size=10, color=BLACK)
    ...
    # line item
    text(LEFT_X, 387, "WhatsApp AI agent setup fee \u2014 instant response, qualification, booking + custom dashboard",
         font=FONT_REGULAR, size=9.5, color=BLACK)
    ...
    paragraphs = [
        ...
        ("term", "Monthly retainer: $800 per month for maintenance & optimization, billed "
                 "on the 1st of each month; due within 5 days of issue; a late fee of "
                 "5% applies to late payments. Payments processed via Stripe.",
         FONT_REGULAR, 8, GREY),
        ("term", "Scope: WhatsApp AI agent (instant response, qualification, booking) + "
                 "custom dashboard for LeadFlow.io, a marketing agency running paid ads "
                 "for local businesses and ecommerce brands (3 years). Fixes slow lead "
                 "response time, inconsistent follow-up, no visibility on lead performance.",
         FONT_REGULAR, 8, GREY),
        ("term", "Platforms: GoHighLevel, n8n, Supabase; leads via Facebook lead forms and "
                 "Instagram DMs. Timeline: 14 days from kickoff \u2014 go live by "
                 "September 5, 2026.",
         FONT_REGULAR, 8, GREY),
        ("term", "Proposal PRO-2026-002 dated August 22, 2026 (discovery call August 20, "
                 "2026), valid until September 22, 2026. Access next: ad account access, "
                 "lead form structure, calendar access.",
         FONT_REGULAR, 8, GREY),
