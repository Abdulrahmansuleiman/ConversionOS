#!/usr/bin/env python3
"""Build the LaunchOps AI Service Agreement (contract.pdf) for LeadFlow.io.

Reconstructs templates/Contract Template.pdf section-for-section using ReportLab.
Every client fact comes VERBATIM from clients/leadflow/facts.json.
Font: Montserrat (fonts/Montserrat/) for ALL text — never Helvetica/Times.

Output : clients/leadflow/pdfs/contract.pdf  (US Letter, multi-page)
"""
...
def draw_cover(c, doc):
    ...
    c.setFillColor(BODY)
    c.drawRightString(534.65, _ytop(157.7, 10.5), "CON-2026-002")
    ...
    values = [
        ...
        "LeadFlow.io",
        "Daniel Whitfield",
        "24 Baker Street, Marylebone, London, W1U 6TU, UK",
        "LaunchOps AI",
        "Raymon (Abdulrahman Suleiman, Founder)",
    ]
    cell_h = 26.0
    for i in range(6):
        top = row_tops[i]
...
def build_story():
    ...
    story.append(Paragraph(
        "The contact details of the Parties are as follows: Client \u2014 Daniel Whitfield, "
        "LeadFlow.io, 24 Baker Street, Marylebone, London, W1U 6TU, UK, "
        "+44 7911 123456. Provider \u2014 LaunchOps AI, "
    ...
    story.append(Paragraph(
        ...
        "agreed during the discovery call of August 20, 2026 and set out in Proposal "
        "No. PRO-2026-002 (dated August 22, 2026, valid until September 22, 2026). "
        "Client is a marketing agency running paid ads for local businesses and "
        "ecommerce brands (3 years in business); today it suffers a slow lead response "
        "time, inconsistent follow-up, and no visibility on lead performance:",
        BODY_ST,
    ))

    rows = [
        ("Agent Type(s)", "Text AI Agent \u2014 WhatsApp AI agent"),
        ("Business Vertical", "Marketing agency \u2014 paid ads for local businesses and ecommerce brands"),
        ("Core Platform(s)", "GoHighLevel (GHL), n8n, Supabase"),
        (
            "Key Deliverables",
            "An instant response to every inbound lead, qualification "
            "(budget/intent/timeline) before booking, booking straight into calendar, "
            "and a custom dashboard showing conversation logs, response times, and "
            "booked call stats",
        ),
        (
            "Integrations",
            "Leads redirected from Facebook lead forms and Instagram DMs to WhatsApp "
            "after form submission; calendar booking; GoHighLevel CRM layer, n8n "
            "automation, Supabase data storage",
        ),
        ("Build Timeline", "14 days from kickoff \u2014 go live by September 5, 2026"),
        (
            "Client Responsibilities",
            "Client shall provide ad account access, lead form structure, and calendar "
            "access following signature of this Agreement",
        ),
    ]
    data = [
    ...
    story.append(Paragraph(
        ...
        "in writing, and is invoiced under Invoice No. INV-2026-002 (payment status: "
        "AWAITING PAYMENT). Provider will issue Receipt No. RCP-2026-002 upon "
        "receipt of payment.",
        BODY_ST,
    ))

    story.append(Paragraph("3.2 Monthly Maintenance & Optimization Fee", SUB))
    story.append(Paragraph(
        "Beginning on the go-live date, Client agrees to pay Provider a recurring "
        "monthly fee of $800 per month for ongoing optimization, monitoring, "
    ...
    for item in (
        "Ongoing performance monitoring and reporting via the custom dashboard",
        "Script and prompt refinement based on real conversation data",
        "Minor adjustments to agent behavior, qualification flow, and objection handling",
...
def build(total_pages=None):
    doc = BaseDocTemplate(
        ...
        author="LaunchOps AI",
        subject="AI Agent Implementation & Optimization Services - LeadFlow.io",
    )
